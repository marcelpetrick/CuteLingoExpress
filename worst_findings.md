...
4. **Fixed: numerus handling preserves Qt TS plural form slots**
   Numerus messages no longer hard-code two plural forms. The current rewrite path preserves the existing `<numerusform>` count from the TS file and fills each existing slot, with a single-slot fallback only for malformed unfinished numerus messages without any slots. This keeps Qt's language-specific plural structure intact, though the machine translation still supplies one translated text that may need native review for linguistic quality.
